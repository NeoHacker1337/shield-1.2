...
const AudioCallScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    webrtcService.onConnectionState = (state) => {
      ...
      // end call on disconnect/close/failed
          ...
          if (!hasAnswered.current) {
            ...
            if (offer) {
              try {
                ...
                // if createAnswer does NOT post to backend, keep this
  ...
  const startCall = async () => {
    try {
      ...
      if (isCaller) {
        // this will init(), createOffer, setLocalDescription and send offer to backend
        await webrtcService.startCaller(roomId, userId);
        setStatus('Calling...');
      } else {
        // receiver only prepares local stream/PC; offer will come from backend
        await webrtcService.init(roomId);
